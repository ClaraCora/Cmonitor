...
import { addresses, changes, GIB, provisioningSite, supportsTerminal, trafficCorrection } from "./api.ts"
...
assert.equal(supportsTerminal("1.1.0"), true)
assert.equal(supportsTerminal("v1.2.3"), true)
assert.equal(supportsTerminal("1.0.9"), false)
assert.equal(supportsTerminal(""), false)
